...
@Component({
  ...
  template: `
    <button type="button" class="text-white bg-indigo-500 border border-transparent hover:bg-indigo-600 focus:ring-indigo-400 disabled:hover:bg-indigo-500 dark:bg-indigo-600 dark:hover:bg-indigo-500 dark:focus:ring-indigo-600 dark:disabled:hover:bg-indigo-600 focus:!ring-2 group flex h-min items-center justify-center p-0.5 text-center font-medium focus:z-10 rounded-lg"
            (click)="toggle()"
            #button>
      <span class="flex items-center rounded-md text-sm px-4 py-2">
        {{ label }}
        <svg stroke="currentColor" fill="none" stroke-width="0" viewBox="0 0 24 24" class="ml-2 h-4 w-4" height="1em" width="1em" xmlns="http://www.w3.org/2000/svg">

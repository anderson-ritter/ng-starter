import { Directive, HostBinding, Input } from '@angular/core';

import { FloatingLabelType, InputValidation, LabelProperties } from '../form-field.properties';
import { BaseInputDirective } from './base-input.directive';

@Directive({
  selector: 'label[flowbiteLabel]',
})
export class LabelDirective extends BaseInputDirective {
  _parentId = '';
  _validation: InputValidation | null = null;
  _floatingLabelType: FloatingLabelType | null = null;

  @HostBinding('attr.for') get for() {
    return this._parentId;
  }

  @Input() set parentId(id: string) {
    this._parentId = id;
  }

  @Input() set validation(validation: InputValidation | null) {
    this._validation = validation;
    this.handleClasses();
  }

  @Input() set floatingLabelType(floatingLabelType: FloatingLabelType | null) {
    this._floatingLabelType = floatingLabelType;
    this.handleClasses();
  }

  override handleClasses(): void {
    this._classes = this._classes ?? [];

    if (this._floatingLabelType) {
      this._classes.push(...LabelProperties.floatingLabel[this._floatingLabelType].base);

      if (this._validation) {
        this._classes.push(...LabelProperties.floatingLabel[this._floatingLabelType].validation[this._validation]);

      } else {
        this._classes.push(...LabelProperties.floatingLabel[this._floatingLabelType].default);
      }

      return;
    }

    this._classes.push(...LabelProperties.default.base);

    if (this._validation) {
      this._classes.push(...LabelProperties.default.validation[this._validation]);

    } else {
      this._classes.push(...LabelProperties.default.default);
    }
  }
}
